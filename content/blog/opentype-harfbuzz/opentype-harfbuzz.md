...
When we are working with source data (as opposed to binary, like `.ttf`), Fontra emulates certain features that would usually be generated by a font compiler, such as `kern` for kerning or `mark` for mark-to-base attachments, among others.
...
If the feature code contains a `kern` (or `curs`, `mark`, `mkmk`) feature block *without* an insertion marker, that feature will *not* be generated, and emulation will be off by default.
...
Implementing all this behavior was quite a challenge. Luckily I got help from [Khaled Hosny](https://khaledhosny.com/), who contributed several key bits of functionality.
...
It returns the compiled font data, error and warning messages, and “insert markers” information.

The “insert markers” data correspond to `# Automatic Code` comments in the feature code, and tells us before which `GPOS` lookups the emulated features should be applied.
...
I would like to take this moment to thank Google Fonts and Dave Crossland for supporting me and Khaled to build this exciting new Fontra functionality.
